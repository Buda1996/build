...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^blackbox_") ; then
       BlackBox_BUILD=$(echo -n $1 | sed -e 's/^blackbox_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $BlackBox_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       BlackBox_BUILD=
    fi
    export BlackBox_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    BlackBox_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/BlackBox/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the BlackBox model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch blackbox_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var BlackBox_VERSION)
        ZIPFILE=BlackBox-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.blackbox.device=$BlackBox_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $BlackBox_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $BlackBox_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/BlackBox/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
